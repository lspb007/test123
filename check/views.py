...
from .models import SystemInfo, LocationDict,UserCheckDev,ResourceInfo,UserCheckSys,Department2System,UserCheckSec,UserCheckNet,UserCheckNetSys,SecurityDevice,NetworkDevice,UserCheckNet,UserCheckNetSys,usercheckNewMedia,NewMedia
...
def index(request):
    ...
    if request.user.is_authenticated():
        ...
        #新媒体
        if request.user.department_id == 6:
...
def check(request):
    ...
    #新媒体
    xin = models.Department2System.objects.filter(d=6).values("s_id")
    print(xin)
    xin_system_count = SystemInfo.objects.filter(id__in=xin).count()
    print(xin_system_count)
    xin_system = models.SystemInfo.objects.filter(id__in=xin)
    print(xin_system)
    xin_department_name = Department.objects.get(id=6)
    if request.user.department_id == 4:
    ...
    if request.user.department_id == 6:
        return render(request, "check_6.html", {
            "all_system": xin_system,
            "system_nums":  xin_system_count,
            "chech_t": chech_t,
        "res_count":res_count,
        "system_check_time":system_check_time,
            "department_name": xin_department_name
        })
...
def detail(request):
    ...
    res_count= check_system.resourceinfo_set.count()
    #新媒体
    all_ins_items = check_system.newmedia_set.all()[:100]
    ins_items_count = check_system.newmedia_set.count()
    ...
    if request.user.is_authenticated():
        ...
        if request.user.department_id == 6:
            return render(request, 'check-detail-homepagenewmedia.html', {
                "all_resource": all_ins_items,
                # "all_teachers": all_teachers,
                "check_system": check_system,
                "system_id": system_id,
                "res_count": ins_items_count,
                "auto_check_info_list": auto_check_info_list,
                "ssh_error": ssh_error
                # "has_fav": has_fav,
...
class AddUserAskView3(View):
    def post(self, request):
        ...
        a = request.POST.get('user_name')
        b = request.POST.getlist('dev_id')
        c = request.POST.getlist('service')
        d = request.POST.getlist('details')

        for i in range(len(b)):
             print(b)
             p1=models.NewMedia.objects.get(id=b[i])
             obj = models.usercheckNewMedia(user_name=a,insitem_id =p1,service=c[i],details=d[i])
             obj.save()
        # u=request.POST.get('user_name')
        # s=request.POST.get('sysisnormal')
        # p=request.POST.get('details')
...
def checknewmediainfo(request):
        dev_check =usercheckNewMedia.objects.order_by("-add_time")
        all_system = SystemInfo.objects.all()
        dev_check_new = usercheckNewMedia.objects.raw("select  check_userchecknewmedia.*,check_systeminfo.`name` from check_userchecknewmedia,check_systeminfo,check_newmedia where  add_time in (select MAX(add_time) from check_userchecknewmedia  GROUP BY insitem_id_id) and check_userchecknewmedia.insitem_id_id=check_newmedia.id and check_newmedia.resource_system_id=check_systeminfo.id ORDER BY check_systeminfo.id")
        sys= UserCheckDev.objects.all()
        print(dev_check_new)
        Resource_count = ResourceInfo.objects.count()
        return render(
            request, 'checknewmedia.html',{
                 "dev_check": dev_check,
                "dev_check_new":dev_check_new,
                "all_system": all_system
                # "Resource": Resource_count,
